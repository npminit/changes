    ...
    Storage.getItem(clanData[clanTag].warId, (err, warData) => {
      ...
      Storage.getItem(`${clanData[clanTag].warId}warCalls`, (err, warCalls) => {
        ...
        if (call[0] == 'empty') {
          ...
          warCalls[number] = `${caller}//${message.author.id}//${moment().format()}`;
          ...
          CLB[clanTag].calls++
          ClanStorage.setItemSync(`callsLeaderBoard`, JSON.stringify(CLB));
          ...
          ClanStorage.getItem('stats', (err, stats) => {
            ...
            list(clanTag, groupConfig.hideBases, (list) => {
