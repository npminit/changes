...
exports.run = (client, message, args, oldClanTag) => {
  ...
  funcs.getCurrentWar(clanTag, (data, clanTag) => {
    ...
    if (data.reason == 'accessDenied.invalidIp') {
      return message.reply('please contact a admin and let us know we need new coc api keys')
        ...
        */
        ...
        if (groupInfo[0] === message.group.id) {
          ...
          .then(() => {
            ...
            message.reply(`this group will now recieve updates for ${data.clan.name} instead of ${clanData[oldClanTag].name}`)
