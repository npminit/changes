...
attackMessage = (WarData, attackData) => {
  ...
  Storage.getItem(`${clanData[WarData.stats.clan.tag].warId}warAttacks`, (err, warAtt) => {
    ...
    Storage.getItem(`${clanData[WarData.stats.clan.tag].warId}warCalls`, (err, warCalls) => {
      ...
      var attackMessage = `${(attackData.who == 'clan') ? `${WarData.stats.clan.name} attacked ${WarData.stats.opponent.name}` : `${WarData.stats.opponent.name} attacked ${WarData.stats.clan.name}`}
      Attacker: \n${attacker.mapPosition}. ${attacker.name}
      ...
      clanData[WarData.stats.clan.tag].channels.forEach((group) => {
        const groupConfig = Client.settings.get(group);

        if (!groupConfig.enemyUpdates && attackData.who == 'opponent') return

        if (groupConfig.hideList) {
          Client.sendMessage(group, attackMessage);
        } else {
          list(WarData.stats.clan.tag, groupConfig.hideList, (list) => {
  ...
  if (war && war.reason != 'notFound' && war.reason != 'accessDenied' && war.state != 'notInWar') {
    ...
    var WarData = Storage.getItem(warId, (err, WarData) => {
      ...
      if (!WarData) {
        ...
        var stats = ClanStorage.getItemSync('stats')

        if (!stats) stats = { callsLogged: 0, warsLogged: 0 }
        stats.warsLogged++

        ClanStorage.setItemSync('stats', stats);

  
      }
      if (clanData[war.clan.tag].onStartup) {
